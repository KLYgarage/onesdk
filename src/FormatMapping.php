...
class FormatMapping {
		...
		if ($this->filterArray($data)) {
			...
			for ($i = 0; $i < $numberOfItems; $i++) {
				...
				$attachment = $this->filterAttachmentObject(
					$this->makeAttachmentObject($attachmentType,
						$attributes, $item));
	...
	private function makeAttachmentObject($attachmentType, $attributes, $item) {
		...
		case self::JSON_PAGE_FIELD:
			return $this->createPage($page_title, $page_body, $page_source,
				$page_order, $page_cover, $page_lead);
		case self::JSON_GALLERY_FIELD:
			return $this->createGallery($gallery_body, $gallery_order,
				$gallery_photo, $gallery_source, $gallery_lead);
		case self::JSON_VIDEO_FIELD:
			return $this->createVideo($video_body, $video_source,
				$video_order, $video_cover, $video_lead);
		default:
			return null;
		}
	}
